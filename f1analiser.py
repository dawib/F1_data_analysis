...
#names of events considered in the analysis
ALL_EVENTS = ['practice-1', 'practice-2', 'practice-3', 'qualifying', 'sprint-results', 'race-result']


def convert(time):
#function to convert time in the form of string to number
#i.e. "1:33.525" -> 93.525
...
#function to convert time loss from string to number
#i.e. "+1.131" -> 1.131
    if time[0]==' ':
        return 0.
    else:
...
#function to convert results form from dictionary to data frame
#the returned data frame has colums
#driver, year, GP, fp1 time, fp1 time loss, fp2 time, fp2 time loss, fp3 time, fp3 time loss, 
#q1 time, q1 time loss, q2 time, q2 time loss, q3 time, q3 time loss, race result
...
def get_weekends(year):
    ...
    #get the list of links to every GP results
...
def get_gp(year,wknd):
    #return the results of a given Grand prix
...
def get_data(Years,verbose=0):
    #returns the data frame of results from given years
...
def write_results_to_file(path,Drivers,Weekends,Links,Results):
    #writes a dictionary of results to a file
...
def load_results_from_file(path):
    #loads a dictionary of results from a file
